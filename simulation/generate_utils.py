...
def infer_audio_new(stn_tst, net_g, sid, device, len_scale):
    with torch.no_grad():
        x_tst = stn_tst.to(device)
        x_tst = x_tst.unsqueeze(0)
        x_tst_lengths = torch.LongTensor([stn_tst.size(0)]).to(device)
        outputs = net_g.infer(x_tst, x_tst_lengths, noise_scale=.667, sid=sid,noise_scale_w=0.8, length_scale=len_scale)
        audio = outputs[0][0,0].data.cpu().float().numpy()
        durations = outputs[-1][0,0] # dim = 1
...
def get_time_transcription(durations, stn_tst):
    ...
    for i, (start, end) in enumerate(timestamps):
        if stn_tst[i] !=0:  ## blank and padding
            ...
            ret.append({
                "phoneme": phoneme,
                "start": round(start.item(), 5),  #
                "end": round(end.item(), 5),   #
                "type": None
            })
